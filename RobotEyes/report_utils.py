...
class ReportUtils:
    ...
    def generate_report(baseline_folder, results_folder, actual_folder=None):
        ...
        relative_baseline_folder_path = ReportUtils.relative_path(baseline_folder, results_folder.replace(os.getcwd(), ''))
        relative_actual_folder_path = ReportUtils.relative_path(actual_folder, results_folder.replace(os.getcwd(), '')) if actual_folder else ''
        ...
        for t in tree.findall('.//test'):
            ...
            if not os.path.exists(os.path.join(img_path, 'actual', folder_name)):
                for open_eyes_keyword in t.findall('.//kw[@name="Open Eyes"]'):
                    ...
                    folder_name = ReportUtils.get_template_test_name(open_eyes_keyword)

                    if folder_name:
                        if actual_folder:
                            html = ReportUtils.make_non_web_test_table(html, baseline_folder, relative_baseline_folder_path, img_path,
                                                           test_name, folder_name, actual_folder,
                                                           relative_actual_folder_path)
                        else:
                            html = ReportUtils.make_test_table(html, baseline_folder, relative_baseline_folder_path, img_path, test_name,
                                            folder_name)
            else:
                if actual_folder:
                    html = ReportUtils.make_non_web_test_table(html, baseline_folder, relative_baseline_folder_path, img_path,
    ...
    def relative_path(baseline_folder, results_folder):
        ...
        if os.path.exists(os.getcwd() + os.path.sep + baseline_folder) and not os.path.isabs(baseline_folder):
            ...
            count = ReportUtils.get_count_of_sub_directories(results_folder)
            s = ''
